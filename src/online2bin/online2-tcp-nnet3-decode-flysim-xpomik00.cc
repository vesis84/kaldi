...
class TcpClient {
  public:
    TcpClient(const char* ip, int port_num);
    ~TcpClient();

    int Write(std::string msg, std::string printed_info);
...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("cl-port", &client_port,
                "Client port.");
    po.Register("cl-ip", &client_ip,
                "Client IP.");
    ...
    TcpClient client(client_ip.c_str(), client_port);
        ...
        while (true) {
          ...
          if (eos) {
            ...
            if (decoder.NumFramesDecoded() > 0) {
              ...
              client.Write(msg, "EndOfAudio send: ");
            }
          ...
          if (samp_count > check_count) {
            if (decoder.NumFramesDecoded() > 0) {
              ...
              KALDI_VLOG(2) << "One-best word sequence: "<< ToString(one_best_prev);
              client.Write(msg, "Sending message: ");
            }
          ...
          if (decoder.EndpointDetected(endpoint_opts)) {
            ...
            KALDI_VLOG(2) << "One-best word sequence: "<< ToString(one_best_prev);
            client.Write(msg, "End Sending msg: ");
...
TcpClient::TcpClient(const char* ip, int port_num){
  // socket preparation, filling network structures
  socket_descriptor_ = socket(AF_INET, SOCK_STREAM, 0);
  remote_addr_.sin_family = AF_INET;
  remote_addr_.sin_port = htons(port_num);

  hp_ = gethostbyname(ip);
  memcpy(&(remote_addr_.sin_addr.s_addr), hp_->h_addr, hp_->h_length);

  if (connect(socket_descriptor_, (struct sockaddr *) &remote_addr_, sizeof(remote_addr_)) != 0){
    socket_descriptor_ = -1;
  }
}

int TcpClient::Write(std::string msg, std::string printed_info){
  std::cout << printed_info << msg << std::endl;
  msg.append("\n");
  ...
  if(msg.length() > 2){
    if(socket_descriptor_ > 0){
      ret_val = write(socket_descriptor_, msg.c_str(), msg.length());
    }
